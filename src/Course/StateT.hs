...
instance Functor k => Functor (StateT s k) where
  (<$>) ::
    ...
    -> StateT s k b
  (<$>) f s_ka =
    StateT (\s -> (\(a, s') -> (f a, s')) <$> runStateT s_ka s)
...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) ::
    ...
    -> StateT s k b
  (<*>) s_kfab s_ka =
    StateT (\s -> (\(f_ab, s') -> (\(a, s'') -> (f_ab a, s'')) <$> runStateT s_ka s') =<< runStateT s_kfab s)
...
instance Monad k => Monad (StateT s k) where
  (=<<) ::
    ...
    -> StateT s k b
  (=<<) f_a_skb s_ka =
    StateT (\s -> (\(a, s') -> (runStateT (f_a_skb a) s')) =<< runStateT s_ka s)
