...
instance Applicative List where
  ...
  (<*>) ::
    ...
    -> List a
    -> List b
  (<*>) list_f list_a = flatten (map (\f -> map f list_a) list_f)
...
instance Applicative Optional where
  ...
  (<*>) ::
    ...
    -> Optional a
    -> Optional b
  (<*>) opt_f opt_a = bindOptional (\f -> mapOptional f opt_a) opt_f
...
lift3 ::
  ...
  -> k c
  -> k d
lift3 f_abc ka kb kc = (lift2 f_abc ka kb) <*> kc
...
lift4 ::
  ...
  -> k d
  -> k e
lift4 f_abc ka kb kc kd = lift3 f_abc ka kb kc <*> kd
...
sequence (ka :. l) = lift2 (:.) ka (sequence l)
...
filtering f (a :. l) = 
  (lift2 (\r -> if r then (:.) a else id)) (f a) (filtering f l)



-- (\b (a :. la) -> if b then a :. la else la)
